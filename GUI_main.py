...
class mainGUI(tkinter.Frame):
    ...
    def __init__(self, root, *args):
        ...
        self.previousButton = tkinter.ttk.Button(self.valuebuttonframe, text='Set previous',
                                                 command=self.read_previous_config)

        # Scrollable entry
        self.entryVideo.focus()
        self.entryVideoScroll = tkinter.ttk.Scrollbar(self.mainframe, orient=tkinter.HORIZONTAL,
        ...
        self.imgframe = tkinter.ttk.Labelframe(self.mainframe, text=' Image treatment options: ', padding=10,
                                               relief='sunken')
        self.entryMedian = tkinter.Spinbox(self.imgframe, from_=1, to=100, increment_=2,
                                           textvariable=self.img_medianblur, width=2)
        self.entryGaussian = tkinter.Spinbox(self.imgframe, from_=1, to=100, increment_=2,
                                             textvariable=self.img_gaussblur, width=2)
        ...
        self.checkEqualizeHist = tkinter.ttk.Checkbutton(self.imgframe, text='Apply Histogram Equalizer',
                                                         variable=self.img_equalize_hist, onvalue=True, offvalue=False)
        self.entryClahe = tkinter.Spinbox(self.imgframe, from_=0, to=100, increment_=1, textvariable=self.img_clahe,
                                          width=2)
        ...
        self.entryBilateralD = tkinter.Spinbox(self.bilateralframe, from_=0, to=10000,
                                               textvariable=self.img_bilateral_d, width=2)
        self.entryBilateralColor = tkinter.ttk.Entry(self.bilateralframe, textvariable=self.img_bilateral_color,
                                                     width=3)
        self.entryBilateralSpace = tkinter.ttk.Entry(self.bilateralframe, textvariable=self.img_bilateral_space,
                                                     width=3)

        ##### Detection and tracking options #####
        self.trkframe = tkinter.ttk.Labelframe(self.mainframe, text=' Detection and tracking options: ', padding=10,
                                               relief='sunken')
        ...
        self.checkFit = tkinter.ttk.Checkbutton(self.videoframe, text='Fit to screen', variable=self.vid_fit,
                                                onvalue=True, offvalue=False)
        self.checkContour = tkinter.ttk.Checkbutton(self.videoframe, text='Show binary image',
                                                    variable=self.vid_contour, onvalue=True, offvalue=False)
        ...
        self.labelFPS = tkinter.ttk.Button(self.videoframe, text='Video FPS',
                                           command=lambda: self.calculate_FPS_window('vid'))
        ...
        self.extraframe = tkinter.ttk.Labelframe(self.mainframe, text=' Extra information parameters: ', padding=10,
                                                 relief='sunken')
        self.extraframe.grid(column=0, row=7, rowspan=2, columnspan=3, sticky=(self.N, self.S, self.E, self.W),
                             pady=(10, 0))

        self.checkSplit = tkinter.ttk.Checkbutton(self.extraframe, text='Split circular touching particles',
                                                  variable=self.extra_split, onvalue=True, offvalue=False)
        ...
        if 'angleJet' in self.trackInfo or 'angleJanus' in self.trackInfo or 'angleJetBubble' in self.trackInfo:
            ...
            self.entryExtraRadi = tkinter.Spinbox(self.extraframe, from_=1, to=10000, textvariable=self.extra_partRadi,
                                                  width=3)
            ...
            if 'angleJetBubble' in self.trackInfo:
                ...
                self.entryExtraWidth = tkinter.Spinbox(self.extraframe, from_=1, to=180,
                                                       textvariable=self.extra_partWidth, width=3)
        ...
        # Video options
        self.videoframe.grid(column=0, row=9, rowspan=2, columnspan=3, sticky=(self.N, self.S, self.E, self.W),
                             pady=(10, 0))
        ...
        self.options['filetypes'] = [('Video files', '.avi'), ('Video files', '.mp4'), ('Video files', '.wmv'),
                                     ('All files', '.*')]
    ...
    def show_preview(self, *args):
        ...
        if not self.isPreviewOn:
            ...
            self.buttonPrevRefresh = tkinter.ttk.Button(self.numframe, image=self.refreshImg,
                                                        command=self.actualize_preview)
            self.buttonPrevRefresh.image = self.refreshImg

            # Scale
            self.scaleFrameNum = tkinter.Scale(self.numframe, from_=1, to=self.vid_totalFrames.get(),
                                               variable=self.preview_frameNum, orient=tkinter.HORIZONTAL)
    ...
    def analyze_frame(self):
        ...
        if 'angleJanus' in self.trackInfo:
            self.TrackClass.vecDict = dfun.get_vector_circle(radi=self.TrackClass.Vars['extra_partRadi'],
                                                             ticks=self.TrackClass.Vars['extra_ang_ticks'])
        ...
        for Mind, M in enumerate(M_list):
            label += 1
            center = tuple(
                self.TrackClass.get_contour_information(M, frame_gray=frame_gray, cnt=cnt_list[Mind], binary=binary))
        ...
        # Write frame
        cv2.putText(frame, str(self.count) + '/' + str(self.vid_totalFrames.get()), (5, 15), self.font, 0.5,
    ...
    def count_total_frames(self, *args):
        ...
        if self.vid_lastFilename.get() != filename or self.vid_totalFrames.get() == 0:
            ...
            self.isCv2Accurate = self.vid_totalFrames.get() == int(
                vid.get(7))  # Can OpenCV compute things like FPS, video position,... accurately?
    ...
    def calculate_FPS_window(self, window, *args):
        ...
        self.fps_labelCalculateFPS = tkinter.ttk.Label(self.fpsframe,
                                                       text='Fill the following information to compute fps:')
        ...
        self.fps_buttonCancel = tkinter.ttk.Button(self.fpsframe, text='Cancel', width=4,
                                                   command=lambda: self.fps_exit(False))
        self.fps_buttonAccept = tkinter.ttk.Button(self.fpsframe, text='Accept', width=7,
                                                   command=lambda: self.fps_exit(True))
        ...
        self.fps_computeFrames = tkinter.ttk.Button(self.fpsframe, image=self.fps_computeFrames_img,
                                                    command=self.fps_compute_frames)
...
class dataAnalysisGUI(mainGUI):
    ...
    def __init__(self, root, Vars={}, infoDict={}):
        ...
        self.correct_deleteDict = {p: '' for p in
                                   self.get_particle_list()}  # { Particle to be corrected: [frames to be deleted]}
        ...
        self.correct_startend = self.get_startend_dic(self.infoDict,
                                                      self.get_particle_list())  # { particle : (start,end)}
        ...
        self.checkAll = tkinter.ttk.Checkbutton(self.mainframe, text='All', variable=self.data_checkAll,
                                                command=lambda: self.check_widget(self.data_checkAll.get(),
                                                                                  self.entryParticles), onvalue=True,
                                                offvalue=False)  # Entry is deactivated when "all" is selected

        ##### Drawing options #####
        self.drawframe = tkinter.ttk.Labelframe(self.mainframe, text=' Drawing options: ', padding=10, relief='sunken')
        self.checkDraw = tkinter.ttk.Checkbutton(self.drawframe, text='Draw trajectory', variable=self.data_draw,
                                                 onvalue=True, offvalue=False, command=self.check_colors)
        ...
        # Randomize
        self.checkRand = tkinter.ttk.Checkbutton(self.drawframe, text='Randomize', variable=self.color_rand,
                                                 onvalue=True, offvalue=False, command=self.update_color_image)
        self.randnumframe = tkinter.ttk.Frame(self.drawframe)
        self.labelRandNum = tkinter.ttk.Label(self.randnumframe, text='#')
        self.entryRandNum = tkinter.Spinbox(self.randnumframe, from_=1, to=255, textvariable=self.color_randNum,
                                            width=2)
        ...
        self.checkColorSpeed = tkinter.ttk.Checkbutton(self.colorSpeedFrame, text='Color change with speed',
                                                       variable=self.color_speed, onvalue=True, offvalue=False,
                                                       command=self.colorspeed_check)
        ...
        self.checkLabel = tkinter.ttk.Checkbutton(self.drawframe, text='Show particle label', variable=self.color_label,
                                                  onvalue=True, offvalue=False)

        # Draw orientation
        self.orientationframe = tkinter.ttk.Frame(self.drawframe)
        self.checkOrientation = tkinter.ttk.Checkbutton(self.orientationframe, text='Show orientation every',
                                                        variable=self.color_orientation, onvalue=True, offvalue=False,
                                                        command=self.check_orientation_widgets)
        self.entryOrientationInterval = tkinter.ttk.Entry(self.orientationframe,
                                                          textvariable=self.color_orientation_interval, width=3)
        ...
        self.buttonFPS = tkinter.ttk.Button(self.unitframe, width=4, text='FPS',
                                            command=lambda: self.calculate_FPS_window('data'))
        self.cancelButton = tkinter.ttk.Button(self.mainframe, text='Cancel', command=lambda: sys.exit())
        ...
        self.entryOrder = tkinter.Spinbox(self.dataframe, from_=2, to=8, increment_=2, width=4,
                                          textvariable=self.data_order)

        self.checkAnglevel = tkinter.ttk.Checkbutton(self.dataframe, text='Compute angle from instantaneous velocity',
                                                     variable=self.data_anglevel, onvalue=True, offvalue=False)
        self.checkAAC = tkinter.ttk.Checkbutton(self.dataframe, text='Compute angular auto-correlation',
                                                variable=self.data_AAC, onvalue=True, offvalue=False)
        ...
        ##### CORRECTING OPTIONS #####
        self.correctorframe = tkinter.ttk.Labelframe(self.mainframe, text=' Correcting options: ', padding=10,
                                                     relief='sunken')
        ...
        self.entryDeleteParticle = tkinter.ttk.Combobox(self.deleteframe, textvariable=self.correct_delParticle,
                                                        width=3, height=10, postcommand=self.update_delParticleList)
        ...
        self.entryJoinParticle1 = tkinter.ttk.Combobox(self.joinframe, textvariable=self.correct_joinParticle1, width=3,
                                                       height=10, postcommand=self.update_delParticleList)
        self.labelJoinParticle2 = tkinter.ttk.Label(self.joinframe, text=' with ')
        self.entryJoinParticle2 = tkinter.ttk.Combobox(self.joinframe, textvariable=self.correct_joinParticle2, width=3,
                                                       height=10, postcommand=self.update_joinParticle2List)
            ...
            for widget in children:
                if widget.winfo_class() != 'TFrame':
                    ...
                    childlistContainer.append(
                        widget.winfo_children())  # If it's a frame widget, add the children widgets to the list for the next loop
    ...
    def update_delStartEndLabel(self, particle, *args):
        ...
        self.correct_delStart.set(startend[0]);
        self.correct_delEnd.set(startend[1])
    ...
    def get_particle_list(self, *args):
        return [part for (part, info) in list(self.infoDict.keys()) if
                info == 'position']  # Returns available particles
...
class chooseMethod(mainGUI):
    ...
    def __init__(self, root):
        ...
        self.radioTrackAuto = tkinter.Radiobutton(self.mainframe, text='Auto tracking', variable=self.trackMethod,
                                                  value='auto')
        self.radioTrackLive = tkinter.Radiobutton(self.mainframe, text='Live tracking', variable=self.trackMethod,
                                                  value='live')
        self.radioTrackManual = tkinter.Radiobutton(self.mainframe, text='Manual tracking', variable=self.trackMethod,
                                                    value='manual')

        self.labelChoose = tkinter.ttk.Label(self.mainframe, text='Choose detection method: ')
        self.buttonGradient = tkinter.Radiobutton(self.mainframe, text='Gradient', variable=self.method,
                                                  value='gradient', indicatoron=1)
        self.buttonThresh = tkinter.Radiobutton(self.mainframe, text='Threshold', variable=self.method, value='thresh',
                                                indicatoron=1)
        self.buttonAbsThresh = tkinter.Radiobutton(self.mainframe, text='Abs. thresh', variable=self.method,
                                                   value='absThresh', indicatoron=1)
        self.buttonBgExtraction = tkinter.Radiobutton(self.mainframe, text='Bg extract', variable=self.method,
                                                      value='bgextr', indicatoron=1)

        self.labelExtraInfo = tkinter.ttk.Label(self.mainframe, text='Extra information: ')
        self.buttonNothing = tkinter.Radiobutton(self.mainframe, text='Nothing', variable=self.angleInfo,
                                                 value='nothing', indicatoron=1)
        self.buttonAngleJet = tkinter.Radiobutton(self.mainframe, text='Angle (jet)', variable=self.angleInfo,
                                                  value='angleJet', indicatoron=1)
        self.buttonAngleJanus = tkinter.Radiobutton(self.mainframe, text='Angle (Janus)', variable=self.angleInfo,
                                                    value='angleJanus', indicatoron=1)
        self.buttonAngleJetBubble = tkinter.Radiobutton(self.mainframe, text='Angle (jet + bubbles)',
                                                        variable=self.angleInfo, value='angleJetBubble', indicatoron=1)
        self.checkAngleZ = tkinter.ttk.Checkbutton(self.mainframe, text='Z angle', variable=self.angleZ, onvalue=True,
                                                   offvalue=False)
...
class gradientGUI(mainGUI):
    ...
    def __init__(self, root, *args):
        ...
        self.variables.update(
            {'sobel_order': self.sobel_order, 'sobel_ksize': self.sobel_ksize, 'sobel_thresh': self.sobel_thresh})
        ...
        ##### Sobel options #####
        self.sobelframe = tkinter.ttk.Labelframe(self.mainframe, text=' Gradient options: ', padding=10,
                                                 relief='sunken')
        # Entries
...
class threshGUI(mainGUI):
    def __init__(self, root, *args):
        ...
        self.variables.update(
            {'thr_BonW': self.thr_BonW, 'thr_size': self.thr_size, 'thr_C': self.thr_C, 'thr_adType': self.thr_adType})
        ...
        self.threshframe = tkinter.ttk.Labelframe(self.mainframe, text=' Thresholding options: ', padding=10,
                                                  relief='sunken')
        # Entries
        self.checkThrBonW = tkinter.ttk.Checkbutton(self.threshframe, text='Black nanomotors', variable=self.thr_BonW,
                                                    onvalue=True, offvalue=False)
        self.entryThrSize = tkinter.Spinbox(self.threshframe, from_=3, to_=99, increment_=2, textvariable=self.thr_size,
                                            width=2)
        self.entryThrC = tkinter.Spinbox(self.threshframe, from_=1, to_=31, increment_=1, textvariable=self.thr_C,
                                         width=2)
...
class absThreshGUI(mainGUI):
    ...
    def __init__(self, root, *args):
        ...
        self.checkThrBonW = tkinter.ttk.Checkbutton(self.threshframe, text='Black nanomotors', variable=self.thr_BonW,
                                                    onvalue=True, offvalue=False)
        self.entryThresh = tkinter.Spinbox(self.threshframe, from_=0, to_=255, increment_=1,
                                           textvariable=self.thr_thresh, width=3)
...
class bgextrGUI(mainGUI):
    ...
    def __init__(self, root, *args):
        ...
        self.variables.update(
            {'bg_thresh': self.bg_thresh, 'bg_actualize': self.bg_actualize, 'bg_bgColor': self.bg_bgColor,
             'bg_refFrame': self.bg_refFrame, 'bg_mode': self.bg_mode, 'bg_alpha': self.bg_alpha})
        self.default_values.update(
            {'bg_thresh': 30, 'bg_open': 1, 'bg_close': 3, 'bg_actualize': 50, 'bg_bgColor': 100, 'bg_refFrame': 1,
             'bg_mode': 'Single Frame', 'bg_alpha': 0.9})

        self.open_config('config_bgextr.cfg')
        self.bg_mode_id = {0: 'Single Frame', 1: 'Total Average',
                           2: 'Dynamic Average'}  # Dict relating tab index with name
        ...
        self.bg_refOK = {'Single Frame': False, 'Total Average': False,
                         'Dynamic Average': False}  # True if the correct reference frame is stored
        ...
        self.bgframe = tkinter.ttk.Labelframe(self.mainframe, text=' Background extraction options: ', padding=10,
                                              relief='sunken')
        ...
        self.entryThresh = tkinter.Spinbox(self.singleframemode, from_=0, to_=255, increment_=1,
                                           textvariable=self.bg_thresh, width=3)
        self.labelBgActualize = tkinter.ttk.Label(self.singleframemode, text='Reference update (frames)')
        self.entryBgActualize = tkinter.Spinbox(self.singleframemode, from_=1, to_=10000, increment_=1,
                                                textvariable=self.bg_actualize, width=3)
        self.labelBgColor = tkinter.ttk.Label(self.singleframemode, text='Background color')
        self.entryBgColor = tkinter.Spinbox(self.singleframemode, from_=0, to_=255, increment_=1,
                                            textvariable=self.bg_bgColor, width=4)
        ...
        self.entryThreshAvRef = tkinter.Spinbox(self.totalavmode, from_=0, to_=255, increment_=1,
                                                textvariable=self.bg_thresh, width=3)

        # Dynamic average reference
        self.labelThreshDynRef = tkinter.ttk.Label(self.dynamicavmode, text='Threshold')
        self.entryThreshDynRef = tkinter.Spinbox(self.dynamicavmode, from_=0, to_=255, increment_=1,
                                                 textvariable=self.bg_thresh, width=3)
        self.labelAlpha = tkinter.ttk.Label(self.dynamicavmode, text='Alpha')
        self.entryAlpha = tkinter.Spinbox(self.dynamicavmode, from_=0, to_=1, increment_=0.01,
                                          textvariable=self.bg_alpha, width=4)
...
class manualTrackGUI(mainGUI):  # , *args, **kwargs):
    ...
    def __init__(self, root, *args, **kwargs):
        ...
        self.cancelButton = tkinter.ttk.Button(self.mainframe, text='Cancel', command=lambda: sys.exit())
        self.previousButton = tkinter.ttk.Button(self.mainframe, text='Set previous values',
                                                 command=lambda: self.read_config('Previous_values'))
        self.goBackButton = tkinter.ttk.Button(self.mainframe, text='Go back', width=10, command=self.go_back)
        ...
        self.checkFit = tkinter.ttk.Checkbutton(self.videoframe, text='Fit to screen', variable=self.vid_fit,
                                                onvalue=True, offvalue=False)
        self.checkLines = tkinter.ttk.Checkbutton(self.videoframe, text='Show lines', variable=self.manual_show_lines,
                                                  onvalue=True, offvalue=False)
        self.labelSkip = tkinter.ttk.Label(self.videoframe, text='Skip frames ')
        self.entrySkip = tkinter.ttk.Entry(self.videoframe, textvariable=self.vid_skip, width=4)
        ...
        self.manualframe = tkinter.ttk.Labelframe(self.mainframe, text=' Manual tracking options: ', padding=10,
                                                  relief='sunken')
        ...
        self.checkAngle = tkinter.ttk.Checkbutton(self.manualframe, text='Indicate orientation',
                                                  variable=self.manual_angle, onvalue=True, offvalue=False)
        ...
        # Video options
        self.videoframe.grid(column=0, row=2, rowspan=2, columnspan=6, sticky=(self.N, self.S, self.E, self.W),
                             pady=(10, 0))
...
class EndScreen(mainGUI):
    ...
    def __init__(self, root):
        ...
        self.postProcessingButton = tkinter.ttk.Button(self.mainframe, text='Go to post-processing',
                                                       command=self.go_back_processing)
